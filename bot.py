...
# 当接收到用户的新消息时，调用此函数（只在真正收到用户消息时调用）
...
def message_listener():
    ...
    while True:
        try:
            ...
            who = msgs.get('chat_name', '')
            chat_type = msgs.get('chat_type', '')
            one_msgs = msgs.get('msg', [])
            
            # 只处理监听列表中的聊天对象
            if who not in user_names:
                logger.debug(f"跳过非监听对象: {who}")
                time.sleep(wait)
                continue
            
            logger.info(f"收到来自 [{who}] 的 {len(one_msgs)} 条消息")
            
            # 只处理最后一条有效消息，跳过时间消息和自己发的消息
            valid_msgs = []
            for msg in one_msgs:
                msgtype = msg.type
                content = msg.content
                ...
                logger.debug(f"  消息: type={msgtype}, sender={repr(sender)}, content={repr(content[:30]) if content else None}")
                
                # 跳过空消息、时间消息、系统消息
                if not content or msgtype in ('time', 'system'):
                    continue
                # 跳过自己发的消息
                if sender == ROBOT_WX_NAME or sender == 'self':
                    logger.debug(f"  跳过自己发的消息")
                    continue
            ...
            content = msg.content
            sender = msg.sender
            logger.info(f'处理消息: [{who}] {sender}: {content[:50]}')
            
            if msgtype not in ('text', 'friend', 'image', 'emoji', 'emotion', 'miniapp'):
                logger.debug(f"非文本消息，忽略! 消息类型: {msgtype}")
                time.sleep(wait)
                continue
            ...
            if chat_type == 'friend':
                ...
                # 群聊：检查是否@机器人 或 接受全部群消息
                if Accept_All_Group_Chat_Messages:
                    if not content.lower().endswith(('.png', '.jpg', '.jpeg', '.gif', '.bmp')):
                        msg.content = "群聊消息[" + sender + "]:" + content
                    if '[动画表情]' in content and ENABLE_EMOJI_RECOGNITION:
                        handle_emoji_message(msg, who)
                    else:
                        handle_wxauto_message(msg, who)
                elif ROBOT_WX_NAME != '' and (bool(re.search(f'@{ROBOT_WX_NAME}\u2005', content))):
                    msg.content = re.sub(f'@{ROBOT_WX_NAME}\u2005', '', content).strip()
                    msg.content = "群聊消息[" + sender + "]:" + msg.content
                    handle_wxauto_message(msg, who)
                else:
                    logger.debug(f"群聊消息未@机器人，跳过: {content}")
...
def handle_wxauto_message(msg, who):
    try:
        ...
        sender = getattr(msg, 'sender', who)  # 获取发送者
        msgtype = getattr(msg, 'type', '')  # 获取消息类型
        ...
        # 检查是否是图片消息 (wxautox4中图片消息type='image', content='图片')
        if msgtype == 'image':
            if ENABLE_IMAGE_RECOGNITION:
                try:
                    # wxautox4: 调用download()下载图片，返回保存路径
                    if hasattr(msg, 'download'):
                        img_path = msg.download()
                        logger.info(f"下载图片成功: {img_path}")
                        is_emoji = False
                        content = None
                    else:
                        content = "[图片]"
        ...
        # 检查是否是表情包消息 (wxautox4中emotion消息type='emotion', content='[动画表情]')
        if msgtype == 'emotion' or (content and "[动画表情]" in content):
        ...
        if content:
            ...
            sender_name = sender or username  # 使用发送者名称
...
def capture_and_save_screenshot(who):
    ...
    try:
        ...
        for window in pyautogui.getAllWindows():
            if who in window.title:
                chat_window = window
                break
        
        if not chat_window:
            logger.error(f'未找到聊天窗口: {who}')
            return None
...
def clean_up_temp_files ():
    ...
    if os.path.isdir("wxautox文件下载"):
        ...
        print(f"目录 wxautox文件下载 已成功删除")
    else:
        print(f"目录 wxautox文件下载 不存在，无需删除")
...
def main():
    ...
    try:
        ...
        logger.info(f"机器人昵称: {ROBOT_WX_NAME}")
